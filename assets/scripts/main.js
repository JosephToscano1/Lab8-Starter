...
async function getRecipes() {
  ...
  let local_recipe_list = JSON.parse(localStorage.getItem("recipes"));
  return local_recipe_list;
  ...
  /**************************/
  const recipePromise = new Promise(async function recipeGet(resolve, reject) {
          try {
            for(let i = 0; i<RECIPE_URLS.length; i++)
              {
                let url = await fetch(RECIPE_URLS[i]);
                let urlJson = await url.json();
                network_Recipes.push(urlJson);
                if(i == RECIPE_URLS.length-1){
                  saveRecipesToStorage(network_Recipes);
                  resolve(network_Recipes);
                }
              }
          } catch (error) {
            console.error(error);
